...
async def start_handler(message: types.Message, state: FSMContext):
    await message.answer(
        "Привет! Я астрологический бот. Нажмите кнопку ниже, "
        "чтобы рассчитать свою карту Джйотиш.",
...
async def process_birth_date(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "Неверный формат даты. Пожалуйста, введите дату в "
            "формате ДД-ММ-ГГГГ или ДД.ММ.ГГГГ"
        )
        return

    await state.update_data(birth_date=birth_date)
    await message.answer(
        "Отлично! Теперь, пожалуйста, введи время рождения "
        "(в формате ЧЧ:ММ:СС)."
    )
...
async def process_birth_time(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "Неверный формат времени. Пожалуйста, введите время в "
            "формате ЧЧ:ММ:СС."
        )
        return
    ...
    await message.answer(
        "Теперь введи свое место рождения (город или координаты)."
    )
...
async def process_location(message: types.Message, state: FSMContext):
    ...
    try:
        locations = geolocator.geocode(
            user_input, exactly_one=False, limit=52, language="ru"
        )
        if not locations:
            await message.answer(
        ...
        for location in locations:
            ...
            if replace_yo_with_e(user_input.lower()) == replace_yo_with_e(
                city_name.lower()
            ):
        ...
        if not exact_match_found:
            await message.answer(
                "Город не найден. Пожалуйста, введите полное название города."
            )
            return
        ...
        for location in locations:
            ...
            region = (
                ...
                if len(location.address.split(",")) > 2
                else ""
            )
        ...
        unique_locations.sort(
            key=lambda loc: loc["address"].split(",")[-1].strip()
        )
...
async def process_page_navigation(
...
async def process_city_selection(
    ...
    try:
        ...
        await callback_query.message.answer(
            f"Не удалось получить адрес города: {e}"
        )
        return
...
async def confirm_and_proceed(
    ...
    if not birth_date or not birth_time or not location:
        await message.answer(
            "Не указаны дата или время рождения. "
    ...
    try:
        ...
        await message.answer(
            "Локация введена некорректно. Пожалуйста, "
            f"введите корректные координаты или название города. {e}"
        )
...
async def save_user_data(
    ...
    try:
        user_data_entry = {
            ...
            "birth_date": (
                datetime.strptime(user_data["birth_date"], "%d-%m-%Y").date()
            ),
            "birth_time": (
                datetime.strptime(user_data["birth_time"], "%H:%M:%S").time()
            ),
...
async def calculate_and_send_chart(
    message: types.Message, user_data: dict, session: AsyncSession
):
    birth_date = user_data["birth_date"]
    birth_time = user_data["birth_time"]
    location = user_data["location"]

    planets_positions, zodiac_signs = await calculate_planet_positions(
        birth_date, birth_time, location
    )
    karakas = await calculate_karakas(planets_positions)
    asc_positions, asc_zodiac_signs = await calculate_asc(
        birth_date, birth_time, location
    )
    ...
    if not ascendant_info:
        await message.answer(
            "Ошибка: Не удалось получить информацию об асценденте."
        )
        return

    asc_sign, asc_deg, asc_min, asc_sec = ascendant_info
    asc_longitude = to_decimal_degrees(asc_deg, asc_min, asc_sec)
    asc_nakshatra, asc_pada = await get_nakshatra_and_pada(
        zodiac_symbols_to_names.get(asc_sign, asc_sign), asc_longitude
    )
    ascendant_string = (
        f"Asc {asc_sign} {asc_deg}˚{asc_min:02d}'{asc_sec:02d}"
        f"{asc_nakshatra} {asc_pada}"
    )

    asc_sign_number = zodiac_to_number.get(asc_sign)
    if not asc_sign_number:
        await message.answer(
            "Ошибка: Не удалось определить номер "
            "знака зодиака для асцендента."
        )
        return

    house_info = await get_house_info(asc_sign, planets_positions)

    chart_image = await draw_north_indian_chart(
        asc_sign_number,
        planets_positions,
    )
    ...
    await message.answer_photo(
        photo=input_file,
        caption="Ваш South Indian Chart.",
    )
    ...
    zodiac_info = "\n".join(
        [
            (
                (
                    f"{symbol:<3} {karakas_by_planet.get(symbol, ' '):<5} "
                    f"{zodiac_sign} {degree:>2}˚{minutes:02d}'{seconds:02d}"
                    f"{nakshatra} {pada}"
                )
                if karakas_by_planet.get(symbol)
                else (
                    f"{symbol:<3} {zodiac_sign} {degree:>2}˚"
            ...
            for symbol, (
                ...
                degree,
                minutes,
                seconds) in zodiac_signs.items()
            if (
                nakshatra := (
                    await get_nakshatra_and_pada(
                        zodiac_symbols_to_names.get(zodiac_sign, zodiac_sign),
                        to_decimal_degrees(degree, minutes, seconds),
                    )
                )[0]
            )
            and (
    ...
    years_remaining, years_passed = calculate_remaining_time(
        moon_degree, starting_planet
    )
